...
class CreatePostViewController: CCViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func uploadAsset(fileURL fileURL: NSURL) {
        ...
        post.assetS3Name = currentUserReference.recordID.recordName + NSUUID().UUIDString
        ...
        publicCloudDatabase.saveRecord(post.cloudRecord) { record, error in
            ...
            uploadRequest.bucket = "clipcomp-contentdelivery-mobilehub-1964794544"
            uploadRequest.key = post.assetS3Name
            ...
            transferManager.upload(uploadRequest).continueWithBlock { (task) -> AnyObject? in
                ...
                guard task.error == nil else {
                    print(task.error!)
                    self.publicCloudDatabase.deleteRecordWithID(post.cloudRecord.recordID) { record, error in
                    }
                    return task
                }
